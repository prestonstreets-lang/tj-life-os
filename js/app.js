...
import { createMasteryAsset, createMasteryMilestone, createMasteryPath, createMasteryResource, createMasterySkill, masteryPathProgress, removeMasteryPath, updateMasteryEntity } from './mastery.js';
import { AUTOMATION_RULES, runAutomationChecks } from './automation.js';
import { MODULE_REGISTRY } from './modules.js';

const app = document.querySelector('#app');
const bootSplash = document.querySelector('#bootSplash');
const toastRegion = document.querySelector('#toastRegion');
const validRoutes = new Set(['home', 'money', 'missions', 'streaming', 'body', 'learning', 'analytics', 'vision', 'career', 'calendar', 'systems', 'settings', 'capture', 'voice']);
...
function bodyView() {
  ...
  return `<main class="view" style="--system:${system('body').accent}">
    ${moduleHeader('body', 'Fitness Command', 'Training, nutrition, hydration, recovery and muscle-building momentum.', `${b.workoutStreak} days`, 'current fitness streak')}
    <section class="section glass wide-card"><div class="section-head"><h2>Today’s Performance Readout</h2><span>Live targets</span></div><div class="macro-grid">
...
function learningView() {
  ...
  return `<main class="view mastery-view" style="--system:${system('learning').accent}">
    ${moduleHeader('learning', 'Self Mastery', 'Build any capability you choose through custom paths, visual roadmaps, skills, resources and milestones.', `${l.weeklyMinutes} min`, l.weeklyTarget ? `${remaining} minutes to this week’s growth target` : 'Set your own weekly mastery target')}
    <section class="glass mastery-command"><div><p class="eyebrow">Open learning system</p><h2>Your growth. Your curriculum.</h2><p>Create a path for anything you want to learn, then shape every skill, milestone and reference around your uploaded roadmap.</p></div><div class="button-row"><button class="primary-button" data-route="learning/new-path">Create learning path</button><button class="secondary-button" data-route="learning/new-asset">Upload roadmap</button><button class="secondary-button" data-route="learning/new-skill">Add skill</button></div></section>
    <section class="section"><div class="section-head"><h2>Mastery Paths</h2><span>${l.paths.length} custom paths</span></div><div class="mastery-path-grid">${l.paths.length?l.paths.map(path=>{const charge=masteryPathProgress(path,l);return `<button class="glass mastery-path-card" style="--path:${path.color}" data-route="learning/path--${path.id}"><span class="mastery-path-icon">⌁</span><div><p class="eyebrow">${escapeHtml(path.category)}</p><h3>${escapeHtml(path.title)}</h3><p>${escapeHtml(path.description||'Build this capability one milestone at a time.')}</p>${progress(charge.percent,100)}<small>${charge.percent}% · ${charge.skills} skills · ${charge.completedMilestones}/${charge.milestones} milestones</small>${ownerChip(path.ownerId)}</div></button>`;}).join(''):`<div class="glass mastery-empty">${emptyState('Design your first learning path','Examples: AI systems, a certification, public speaking, music, investing or a new career skill.','learning/new-path','Create a path')}</div>`}</div></section>
    <section class="section"><div class="section-head"><h2>Visual Roadmap Library</h2><button class="section-link" data-route="learning/new-asset">Upload infographic or roadmap</button></div><div class="mastery-roadmap-grid">${l.assets.length?l.assets.map(asset=>`<button class="glass mastery-roadmap" data-route="learning/asset--${asset.id}"><div>${visionImage(asset,'mastery-roadmap-image')}</div><span>${escapeHtml(asset.type)}</span><h3>${escapeHtml(asset.title)}</h3><p>${escapeHtml(asset.notes||'Visual learning reference')}</p>${ownerChip(asset.ownerId)}</button>`).join(''):`<div class="glass mastery-empty">${emptyState('No visual roadmaps yet','Upload a JPEG, PNG or WebP infographic and connect it to a learning path.','learning/new-asset','Upload roadmap')}</div>`}</div></section>
    <div class="module-grid section"><div><section><div class="section-head"><h2>Skill Matrix</h2><button class="section-link" data-route="learning/new-skill">Add skill</button></div><div class="glass list-panel">${l.skills.length?[...l.skills].sort((a,b)=>(a.order||0)-(b.order||0)).map(item=>`<button class="data-row" style="--system:${system('learning').accent}" data-route="learning/skill--${item.id}"><span class="row-icon">${item.level}</span><span><span class="row-title">${escapeHtml(item.title)}</span><span class="row-detail">${escapeHtml(item.description||'Custom skill')} · ${item.xp} XP</span></span><span class="row-meta">${ownerChip(item.ownerId)}<strong>Lv ${item.level}/${item.targetLevel||5}</strong></span></button>`).join(''):emptyState('No skills yet','Add any subject or capability you want to master.','learning/new-skill','Add skill')}</div></section><section class="section"><div class="section-head"><h2>Notes & Resources</h2><button class="section-link" data-route="learning/new-resource">Add resource</button></div><div class="glass list-panel">${l.resources.length?l.resources.map(item=>`<button class="data-row" data-route="learning/resource--${item.id}"><span class="row-icon">⌁</span><span><span class="row-title">${escapeHtml(item.title)}</span><span class="row-detail">${escapeHtml(item.type)}${item.notes?` · ${escapeHtml(item.notes)}`:''}</span></span><span class="row-meta">${ownerChip(item.ownerId)}<span>›</span></span></button>`).join(''):emptyState('No resources yet','Save courses, notes, books, videos or projects.','learning/new-resource','Add resource')}</div></section></div><aside><section class="glass mastery-milestones"><div class="section-head"><h2>Milestones</h2><button class="section-link" data-route="learning/new-milestone">Add</button></div>${l.milestones.length?l.milestones.map(item=>`<button data-route="learning/milestone--${item.id}" class="${item.status}"><i>${item.status==='complete'?'✓':'◇'}</i><span><strong>${escapeHtml(item.title)}</strong><small>${item.targetDate?dayLabel(item.targetDate):'No target date'} · ${escapeHtml(owner(item.ownerId).label)}</small></span></button>`).join(''):`<div class="mastery-small-empty">Turn the roadmap into reachable checkpoints.</div>`}</section>${l.milestone?`<section class="glass mastery-legacy-goal"><span class="card-label">Current focus</span><h3>${escapeHtml(l.milestone)}</h3>${progress(l.weeklyMinutes,l.weeklyTarget)}</section>`:''}</aside></div>
  </main>`;
}
...
function systemsView() {
  return `<main class="view">${viewHeader('System Matrix', 'Open a complete visual command center for each area of household growth.', 'All operational systems')}<div class="stats-grid">${MODULE_REGISTRY.map(module => { const info=system(module.id); return `<button class="glass interactive-card module-hero" style="--system:${info.accent};min-height:210px" data-route="${module.id}"><div><p class="eyebrow" style="color:${info.accent}">${info.eyebrow} · V${module.version}</p><h2 style="font-size:32px">${info.label}</h2><p>${escapeHtml(module.capability)}</p><small>Open system command ›</small></div></button>`; }).join('')}</div></main>`;
}
...
function settingsView() {
  ...
  return `<main class="view">${viewHeader('Settings', 'Personalize the household, motion level and local data controls.', 'Command configuration')}
    ...
    <section class="glass settings-section"><div class="section-head"><h2>Local Automations</h2><button class="section-link" data-action="run-automations">Run checks now</button></div><p class="metric-note">Rules run only on this device and never change money, missions, or job status automatically.</p><div class="automation-grid">${AUTOMATION_RULES.map(rule=>{const enabled=state.automationRules.some(item=>item.id===rule.id&&item.enabled);return `<button class="automation-rule ${enabled?'active':''}" data-action="toggle-automation" data-id="${rule.id}"><i>${enabled?'✓':'○'}</i><span><strong>${escapeHtml(rule.title)}</strong><small>${escapeHtml(rule.detail)}</small></span></button>`;}).join('')}</div>${state.automationHistory.length?`<div class="storage-note">Latest signal: ${escapeHtml(state.automationHistory[0].title)} · ${timeLabel(state.automationHistory[0].occurredAt)}</div>`:''}</section>
    <section class="glass settings-section"><div class="section-head"><h2>Future Connection Layer</h2><span>Adapter-ready</span></div><div class="connection-grid"><div><i>✓</i><strong>Local storage</strong><small>Connected</small></div><div><i>◇</i><strong>Cloud sync</strong><small>Prepared, not connected</small></div><div><i>◇</i><strong>Calendar sync</strong><small>Prepared, not connected</small></div><div><i>◇</i><strong>AI services</strong><small>Prepared, not connected</small></div></div></section>
...
function masteryPathOptions(selected) {
  return `<option value="">Unassigned / general mastery</option>${state.learning.paths.map(path=>`<option value="${path.id}" ${path.id===selected?'selected':''}>${escapeHtml(path.title)}</option>`).join('')}`;
}

function masteryEditorSheet(id) {
  const [kind, entityId] = id.startsWith('new-') ? [id.slice(4), ''] : id.split('--');
  const collections={path:'paths',skill:'skills',asset:'assets',resource:'resources',milestone:'milestones'};
  const collection=collections[kind];
  if(!collection)return null;
  const item=entityId ? state.learning[collection].find(entry=>entry.id===entityId) : null;
  const title={path:'Learning Path',skill:'Skill',asset:'Visual Roadmap',resource:'Resource',milestone:'Milestone'}[kind];
  let fields='';
  if(kind==='path') fields=`<label class="field-label">Path name<input class="field" name="title" required maxlength="70" value="${escapeHtml(item?.title||'')}" placeholder="AI systems, public speaking, investing…" /></label><label class="field-label">Category<input class="field" name="category" maxlength="40" value="${escapeHtml(item?.category||'Personal Growth')}" /></label><label class="field-label">Description<textarea class="field" name="description" rows="3" maxlength="240">${escapeHtml(item?.description||'')}</textarea></label><label class="field-label">Ownership<select class="field" name="ownerId">${memberOptions(item?.ownerId)}</select></label><label class="field-label">Status<select class="field" name="status">${['active','paused','complete'].map(value=>`<option value="${value}" ${value===(item?.status||'active')?'selected':''}>${value}</option>`).join('')}</select></label><label class="field-label">Path color<input class="field mastery-color" name="color" type="color" value="${escapeHtml(item?.color||'#63e6ff')}" /></label><label class="field-label">Target date<input class="field" name="targetDate" type="date" value="${item?.targetDate||''}" /></label><label class="field-label">Weekly minutes goal<input class="field" name="weeklyGoal" type="number" min="0" max="10000" value="${item?.weeklyGoal??60}" /></label>`;
  if(kind==='skill') fields=`<label class="field-label">Skill name<input class="field" name="title" required maxlength="70" value="${escapeHtml(item?.title||'')}" placeholder="Prompt engineering, Python, negotiation…" /></label><label class="field-label">Learning path<select class="field" name="pathId">${masteryPathOptions(item?.pathId)}</select></label><label class="field-label">Description<textarea class="field" name="description" rows="3" maxlength="220">${escapeHtml(item?.description||'')}</textarea></label><label class="field-label">Ownership<select class="field" name="ownerId">${memberOptions(item?.ownerId)}</select></label><label class="field-label">Status<select class="field" name="status">${['active','paused','mastered'].map(value=>`<option value="${value}" ${value===(item?.status||'active')?'selected':''}>${value}</option>`).join('')}</select></label><label class="field-label">Current level<input class="field" name="level" type="number" min="1" max="100" value="${item?.level??1}" /></label><label class="field-label">Current XP<input class="field" name="xp" type="number" min="0" value="${item?.xp??0}" /></label><label class="field-label">Target level<input class="field" name="targetLevel" type="number" min="1" max="100" value="${item?.targetLevel??5}" /></label>`;
  if(kind==='asset') fields=`${item?.mediaId?`<div class="mastery-editor-preview">${visionImage(item,'mastery-roadmap-image')}</div>`:''}<label class="field-label">Infographic / roadmap image<input class="field" name="image" type="file" accept="image/jpeg,image/png,image/webp" ${item?'':'required'} /></label><label class="field-label">Title<input class="field" name="title" required maxlength="70" value="${escapeHtml(item?.title||'')}" placeholder="My complete AI roadmap" /></label><label class="field-label">Visual type<select class="field" name="type">${['infographic','roadmap','reference'].map(value=>`<option value="${value}" ${value===(item?.type||'roadmap')?'selected':''}>${value}</option>`).join('')}</select></label><label class="field-label">Learning path<select class="field" name="pathId">${masteryPathOptions(item?.pathId)}</select></label><label class="field-label">Ownership<select class="field" name="ownerId">${memberOptions(item?.ownerId)}</select></label><label class="field-label">Accessible image description<input class="field" name="altText" maxlength="140" value="${escapeHtml(item?.altText||'')}" placeholder="Describe the roadmap" /></label><label class="field-label">Notes<textarea class="field" name="notes" rows="3" maxlength="300">${escapeHtml(item?.notes||'')}</textarea></label>`;
  if(kind==='resource') fields=`<label class="field-label">Resource title<input class="field" name="title" required maxlength="90" value="${escapeHtml(item?.title||'')}" /></label><label class="field-label">Type<input class="field" name="type" maxlength="30" value="${escapeHtml(item?.type||'Course')}" placeholder="Course, book, note, video…" /></label><label class="field-label">URL<input class="field" name="url" type="url" value="${escapeHtml(item?.url||'')}" placeholder="https://…" /></label><label class="field-label">Learning path<select class="field" name="pathId">${masteryPathOptions(item?.pathId)}</select></label><label class="field-label">Ownership<select class="field" name="ownerId">${memberOptions(item?.ownerId)}</select></label><label class="field-label">Notes<textarea class="field" name="notes" rows="4" maxlength="500">${escapeHtml(item?.notes||'')}</textarea></label>`;
  if(kind==='milestone') fields=`<label class="field-label">Milestone title<input class="field" name="title" required maxlength="90" value="${escapeHtml(item?.title||'')}" /></label><label class="field-label">Learning path<select class="field" name="pathId">${masteryPathOptions(item?.pathId)}</select></label><label class="field-label">Target date<input class="field" name="targetDate" type="date" value="${item?.targetDate||''}" /></label><label class="field-label">Status<select class="field" name="status">${['planned','in-progress','complete'].map(value=>`<option value="${value}" ${value===(item?.status||'planned')?'selected':''}>${value}</option>`).join('')}</select></label><label class="field-label">Ownership<select class="field" name="ownerId">${memberOptions(item?.ownerId)}</select></label>`;
  return `<div class="sheet-wrap" data-action="close-overlay"><form class="sheet mastery-sheet" id="masteryForm" data-sheet><div class="sheet-handle"></div><header class="sheet-header"><div><p class="eyebrow">Self Mastery editor</p><h2>${item?'Update':'Add'} ${title}</h2></div><button type="button" class="sheet-close" data-action="close-overlay" aria-label="Close">×</button></header><input type="hidden" name="entity" value="${kind}"/><input type="hidden" name="id" value="${item?.id||''}"/><input type="hidden" name="mediaId" value="${item?.mediaId||''}"/><div class="field-grid mastery-form-grid">${fields}</div><div class="button-row"><button class="primary-button" type="button" data-action="save-mastery">${item?'Save changes':`Add ${title.toLowerCase()}`}</button>${item?`<button class="danger-button" type="button" data-action="delete-mastery" data-entity="${kind}" data-id="${item.id}">Delete</button>`:''}</div><p class="storage-note">Changes and uploaded images stay in the app’s storage layer and are included in full backups.</p></form></div>`;
}

function detailSheet(base, id) {
  if (base==='vision') return visionSheet(id);
  if (base==='learning' && (id.startsWith('new-')||id.includes('--'))) return masteryEditorSheet(id);
...
function hydrateVisionMedia() {
  ...
  const load=async image=>{ const id=image.dataset.visionMedia; try{const record=await store.getMedia(id); if(!record?.blob){image.closest('.vision-photo,.vision-editor-preview,.vision-detail-hero,.home-vision-media,.mastery-roadmap,.mastery-editor-preview')?.classList.add('media-missing');return;} const url=URL.createObjectURL(record.blob);visionMediaUrls.set(image,url);image.src=url;}catch{image.closest('.vision-photo,.vision-editor-preview,.vision-detail-hero,.home-vision-media,.mastery-roadmap,.mastery-editor-preview')?.classList.add('media-missing');}};
...
async function hydrateMediaUsage() {
  ...
  try { const usage=await store.mediaUsage(); target.textContent=`Media vault: ${usage.count} image${usage.count===1?'':'s'} · ${(usage.bytes/1048576).toFixed(1)} MB${usage.quota?` · browser storage ${((usage.browserUsage||0)/1048576).toFixed(1)} of ${(usage.quota/1048576).toFixed(0)} MB`:''}`; }
  catch { target.textContent='Media-vault usage is unavailable in this browser. Metadata remains accessible.'; }
}
...
function render() {
  ...
  const views = { home: homeView, money: moneyView, missions: missionsView, streaming: streamingView, body: bodyView, learning: learningView, analytics: analyticsView, vision: visionBoardView, career: careerView, calendar: calendarView, systems: systemsView, settings: settingsView };
  applyTheme();
...
function closeOverlay() {
  ...
  if (overlayReturn) { overlayReturn = null; history.back(); return; }
  if (['capture','voice'].includes(route.base)) { go('home', true); return; }
  if (route.detail) { go(route.base, true); return; }
}
...
async function saveMasteryForm(form) {
  const data=new FormData(form);const kind=String(data.get('entity')||'');const collection={path:'paths',skill:'skills',asset:'assets',resource:'resources',milestone:'milestones'}[kind];
  if(!collection)return toast('This Self Mastery item could not be routed.');
  const id=String(data.get('id')||'')||uid(`mastery-${kind}`);const existing=state.learning[collection].find(item=>item.id===id);const now=new Date().toISOString();const ownerId=String(data.get('ownerId')||'household');
  const common={title:String(data.get('title')||'').trim(),ownerId};let patch={...common};let mediaMeta=null;
  if(kind==='path')patch={...patch,description:String(data.get('description')||'').trim(),category:String(data.get('category')||'Personal Growth').trim(),status:String(data.get('status')||'active'),color:String(data.get('color')||'#63e6ff'),targetDate:String(data.get('targetDate')||''),weeklyGoal:Math.max(0,Number(data.get('weeklyGoal'))||0)};
  if(kind==='skill')patch={...patch,pathId:String(data.get('pathId')||'')||null,description:String(data.get('description')||'').trim(),status:String(data.get('status')||'active'),level:Math.max(1,Number(data.get('level'))||1),xp:Math.max(0,Number(data.get('xp'))||0),targetLevel:Math.max(1,Number(data.get('targetLevel'))||5)};
  if(kind==='resource')patch={...patch,pathId:String(data.get('pathId')||'')||null,type:String(data.get('type')||'Resource').trim(),url:String(data.get('url')||'').trim(),notes:String(data.get('notes')||'').trim()};
  if(kind==='milestone'){const status=String(data.get('status')||'planned');patch={...patch,pathId:String(data.get('pathId')||'')||null,targetDate:String(data.get('targetDate')||''),status,completedAt:status==='complete'?(existing?.completedAt||now):null};}
  if(kind==='asset'){
    const altText=String(data.get('altText')||'').trim();const file=data.get('image');
    try{if(file?.size)mediaMeta=await persistSelectedImage(file,altText,ownerId,'Self Mastery');else if(!existing)throw new Error('Choose an infographic, roadmap, or reference image.');}catch(error){return toast(error.message);}
    patch={...patch,pathId:String(data.get('pathId')||'')||null,type:String(data.get('type')||'roadmap'),notes:String(data.get('notes')||'').trim(),altText,mediaId:mediaMeta?.id||existing?.mediaId||null};
  }
  const creators={path:createMasteryPath,skill:createMasterySkill,asset:createMasteryAsset,resource:createMasteryResource,milestone:createMasteryMilestone};const oldMediaId=kind==='asset'?existing?.mediaId:null;
  store.update(next=>{if(existing)next.learning[collection]=updateMasteryEntity(next.learning[collection],id,patch,now);else next.learning[collection].push(creators[kind]({...patch,id},now));if(mediaMeta){next.visionMedia.push(mediaMeta);if(oldMediaId)next.visionMedia=next.visionMedia.filter(item=>item.id!==oldMediaId);}next.activity.unshift({id:uid('activity'),title:`${existing?'Updated':'Added'} ${common.title}`,detail:`Self Mastery ${kind}`,system:'learning',ownerId,entityId:id,occurredAt:now,dataOrigin:'user'});});
  if(mediaMeta&&oldMediaId)await store.deleteMedia(oldMediaId);toast(`${common.title} ${existing?'updated':'added'} in Self Mastery.`);go('learning',true);
}
...
async function handleAction(button) {
  ...
  if (action === 'save-mastery') {
    const form=button.closest('#masteryForm');if(!form?.reportValidity())return;
    try{return await saveMasteryForm(form);}catch(error){console.error('Self Mastery save failed',error);return toast('Self Mastery could not save this item. Your previous data is unchanged.');}
  }
  if (action === 'delete-mastery') {
    const kind=button.dataset.entity;const collection={path:'paths',skill:'skills',asset:'assets',resource:'resources',milestone:'milestones'}[kind];const item=collection&&state.learning[collection].find(entry=>entry.id===id);
    if(!item||!confirm(`Delete “${item.title}”? This cannot be undone.`))return;
    const mediaId=kind==='asset'?item.mediaId:null;
    store.update(next=>{if(kind==='path')next.learning=removeMasteryPath(next.learning,id);else next.learning[collection]=next.learning[collection].filter(entry=>entry.id!==id);if(mediaId)next.visionMedia=next.visionMedia.filter(entry=>entry.id!==mediaId);next.activity.unshift({id:uid('activity'),title:`Deleted ${item.title}`,detail:`Self Mastery ${kind}`,system:'learning',ownerId:item.ownerId,entityId:id,occurredAt:new Date().toISOString(),dataOrigin:'user'});});
    if(mediaId)await store.deleteMedia(mediaId);toast(`${item.title} removed.`);return go('learning',true);
  }
  ...
  if (action === 'toggle-automation') {
    store.update(next=>{const rule=next.automationRules.find(item=>item.id===id);if(rule)rule.enabled=!rule.enabled;else next.automationRules.push({id,enabled:true,updatedAt:new Date().toISOString()});});
    return toast(`${AUTOMATION_RULES.find(rule=>rule.id===id)?.title||'Automation'} ${state.automationRules.find(rule=>rule.id===id)?.enabled?'enabled':'disabled'}.`);
  }
  if (action === 'run-automations') {
    const signals=runAutomationChecks(state,new Date());
    if(!signals.length)return toast('Automation check complete. No new signals.');
    store.update(next=>{signals.forEach(signal=>{next.automationHistory.unshift({id:uid('automation'),...signal});next.activity.unshift({id:uid('activity'),title:signal.title,detail:signal.detail,system:signal.system,ownerId:signal.ownerId,occurredAt:signal.occurredAt,dataOrigin:'automation'});});});
    return toast(`${signals.length} new automation signal${signals.length===1?'':'s'} added.`);
  }
...
app.addEventListener('submit', async event => {
  ...
  if (event.target.id === 'masteryForm') await saveMasteryForm(event.target);
});
...
app.addEventListener('change', event => {
  if (event.target.id === 'resumeJobSelect') store.update(next => { next.career.activeJobId=event.target.value || null; });
});
